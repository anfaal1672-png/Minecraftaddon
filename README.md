...
バニラのTNTに合わせた 16×16 の構成にしてある。板の継ぎ目のような縦線は入れず、
一様な地の色に細かい斑点を散らした面に、中央の4ドットだけが帯になる
（0〜4行が本体、5行が帯のふち、6〜9行が帯、10行がふち、11〜15行が本体）。
本体の色は種類ごとに変え、帯には内容がわかる紋章（4行×12列）を描く。
上面・底面はバニラ同様に帯も文字も無い斑点だけの面で、色で見分ける。
