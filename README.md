...
Minecraft 統合版 (Bedrock) 向けのアドオン。**120種類**のユニークなTNTに加えて、
**投げる爆弾・道具・仕掛けブロック**、そしてゲーム内のTNT図鑑と設定画面を追加します。
...
TNTは「置いて火を点ける」ものなので、置く余裕が無い場面では使えません。
その隙間を埋めるものを別に用意しています。

### 投げる爆弾

投げて当たった場所で効果が出ます。追われている最中でも使えます。

| | 内容 |
| --- | --- |
| 手榴弾 | 当たった場所で爆発する |
| 焼夷手榴弾 | 当たった場所を火の海に変える |
| 閃光弾 | **何も壊さない**。目と足を一時的に潰すだけ |
| 煙玉 | 濃い煙が居座る。逃げるときに足元へ |
| 粘着爆弾 | 貼り付いて3秒後に大きく爆発する |

### 道具

| | 内容 |
| --- | --- |
| リモート起爆装置 | 視線の先のTNT1個を、離れた場所から着火する |
| 一斉起爆ロッド | 視線の先を中心に、半径10のTNTを**全部**着火する |
| 時限装置 | TNTに使うと、5秒後 (使うたび10秒→30秒) に着火する予約を仕掛ける。もう一度使うと解除 |
| 爆発物探知機 | 半径12に埋まっているTNT・**地雷**・鉱石を数えて教える |
| TNT図鑑 | 全種類の威力・効果・レシピを調べる。設定もここから |

### 仕掛けブロック

| | 内容 |
| --- | --- |
| 起爆装置 | レッドストーン信号を受けると、半径12のTNTを一斉に着火する |
| 導火線 | 火を点けると隣の導火線へ燃え広がり、行き着いた先のTNTを着火する |
| 耐爆ブロック | このアドオンのどのTNTでも壊れない。実験場の壁に |
...
data/                     TNTの定義。ここが唯一の情報源
  ...
  gear.mjs                TNT以外の追加物 (投げる爆弾・道具・仕掛けブロック)
...
「アドオンのTNTを、エンジンから見てバニラのTNTそのものとして扱わせる」ことは
**できません**。理由ははっきりしていて、

- バニラの**ブロック**はエンジンに焼き込まれていて、定義ファイルが存在しません
  ([bedrock-samples](https://github.com/Mojang/bedrock-samples) の
  `behavior_pack/blocks/` は空です)
- ブロックのコンポーネント一覧 (`documentation/Blocks.html` の全48種) にも、
  「このブロックをTNTとして扱う」ものはありません
- ディスペンサーの動作表もエンジン側にあり、アイテム側から入る手段がありません

そこで**結果の側から合わせて**います。現在そろっているもの:

| | どう合わせているか |
| --- | --- |
| 着火 (火打石) | **操作そのものを取り消して火を置かせず**、耐久を1減らして `fire.ignite` を鳴らす |
| 着火 (炎・溶岩) | 隣接6方向を定期処理で見る |
| 着火 (レッドストーン) | `getRedstonePower` を定期処理で見る |
| 着火 (燃えている矢) | `projectileHitBlock` で拾う |
| 着火 (他の爆発) | `beforeEvents.explosion` を全部拾い、半径4のTNTを短い導火線で着火 |
| **ディスペンサー** | **正面に吐き出されたTNTのアイテムを捉えて、火の点いたTNTに差し替える** |
| 設置・破壊の音 | バニラと同じ `"sound": "grass"` (実物の `blocks.json` で確認) |
| 導火線の音・見た目 | `random.fuse` と、白く明滅するオーバーレイ |
| 起爆中の当たり判定・物理・ピストン | バニラの `tnt.json` と同じコンポーネント |
| 爆発に巻き込まれたときの導火線 | バニラと同じ `from_explosion` (0.5〜2秒) |
| 燃え移り | バニラと同じ `catch_chance_modifier: 15` |

**ディスペンサー対応**の仕組みはこうです。ディスペンサーは知らないアイテムを
ただのアイテムとして吐き出すので、

1. アイテムが湧いた瞬間 (`entitySpawn`) を捉える
2. それがこのアドオンのTNTで、
3. 隣のディスペンサーがちょうどそちらを向いているなら

そのアイテムを消して、代わりに火の点いたTNTを撃ち出します。
手で落としただけのTNTや、向きの合っていないディスペンサーには反応しません。
ドロッパーは本家でもTNTをそのまま落とすだけなので、こちらも何もしません。

なお **`minecraft:destructible_by_explosion` を 1200 (実質無敵) にしてある**のは
わざとです。バニラのTNTは爆発で「壊される」のではなく「着火される」ので、
壊れないようにしたうえで、連鎖着火をスクリプト側で受け持っています。

### 火打石でTNTが消えていた話

`minecraft:flammable` の `destroy_chance_modifier` を `0` にしてあるのも、
同じくわざとです。公式の説明にこうあります。

> `catch_chance_modifier` が 0 より大きい場合、火はブロックが壊れるまで燃え続ける
> (`destroy_chance_modifier` が 0 なら燃え続けたままになる)

つまり **0 以外にすると、火の点いたTNTブロックは着火ではなく燃え尽きて消えます**。
本家のTNTは火が触れると「着火する」特別扱いを受けますが、アドオンのブロックに
それは無く、ただ燃えて無くなるだけです。

そして火打石は、叩いた面に**火のブロックを置きます**。本家のTNTに使ったときは
火を置かずにTNTが点くだけですが、アドオンのブロックにはその特別扱いも無いので、
置かれた火がTNTに燃え移り、燃え尽きて消えていました。火の燃え移りは確率なので、
**消えたり消えなかったり**という形になります。

直し方は2つです。

1. `destroy_chance_modifier: 0` にして、**絶対に燃え尽きないようにする**
   (火が隣に来たことはブロックの定期処理が拾って10tick以内に着火します)
2. 火打石の操作を `beforeEvents` で**取り消して、そもそも火を置かせない**。
   取り消したうえで、本家と同じ結果 (TNTに点火・耐久を1減らす) をこちらで作ります

あわせて、右クリック1回でイベントが2回来る点も直しました
(`isFirstEvent` を見ないと耐久が2つ減ります)。
さらに保険として、**起爆中のTNTを出せなかった場合はブロックを元に戻します**。
どの経路でも「火を点けたらTNTが消えただけ」にはなりません。
...
**中身は [bedrock-samples](https://github.com/Mojang/bedrock-samples) の
`behavior_pack/entities/tnt.json` と同じもの**にしてあります。
コンポーネントの構成・値・並びまで揃えてあり、連鎖着火で導火線が短くなる
仕組みも、イベント名 (`from_explosion`) までバニラと同じです。
足しているのは「種類ごとの見た目」と「種類ごとの導火線」だけです。

バニラと違うのは1点だけで、`minecraft:damage_sensor` を付けて
**ダメージを完全に無効化**しています。本家の起爆中TNTは体力を持たないので
殴っても矢が当たっても死にませんが、アドオンのエンティティは何かの拍子に
ダメージを受けて「爆発せずに消える」ことがあるためです。

あわせて、爆発の効果が**他の起爆中TNTを対象にしない**ようにしています
...
「火を点けたのにTNTが消えただけで何も起きない」という、いちばん分かりにくい
壊れ方を防ぐためのものです。チャンクが読み込まれていないだけの場合は
TNTも一緒に眠っているので、そこには手を出しません。
